...
// กำหนดตัวแปรสำหรับแสดงข้อมูล
const profileImageUrl = ref(store.profileImageUrl)
const idCardImageUrl = ref("") // รูปบัตรประชาชน
const selfieWithIdCardUrl = ref("") // รูปถ่ายตัวเองถือบัตรประชาชน
const citizenId = ref("") // หมายเลขบัตรประชาชน
...
const loadGuideApplicationsData = async () => {
  try {
    ...
    if (!querySnapshot.empty) {
      ...
      // แปลง Base64 ให้แสดงผลในรูปแบบ URL
      idCardImageUrl.value = `data:image/jpeg;base64,${firstGuideData.citizenCardImage}` || ''
      selfieWithIdCardUrl.value = `data:image/jpeg;base64,${firstGuideData.selfieWithCitizenCardImage}` || ''
      citizenId.value = firstGuideData.citizenId || ''
    } else {
...
const handleFileUpload = (event, imageType) => {
  ...
  if (file) {
    ...
    reader.onload = (e) => {
      if (imageType === 'idCard') {
        idCardImageUrl.value = e.target.result // อัปโหลดรูปบัตรประชาชน
      } else if (imageType === 'selfie') {
        selfieWithIdCardUrl.value = e.target.result // อัปโหลดรูปตัวเองถือบัตรประชาชน
      }
    }
...
const saveIDCardInfo = () => {
  const idCardInfo = {
    idCardImageUrl: idCardImageUrl.value,
    selfieWithIdCardUrl: selfieWithIdCardUrl.value
  }
  localStorage.setItem('idcard-info', JSON.stringify(idCardInfo))
  alert('ข้อมูลถูกบันทึกเรียบร้อยแล้ว')
}

// โหลดข้อมูลจาก localStorage เมื่อหน้าเว็บถูกโหลด
onMounted(() => {
  let idCardInfo = localStorage.getItem('idcard-info')
  if (idCardInfo) {
    idCardInfo = JSON.parse(idCardInfo)
    idCardImageUrl.value = idCardInfo.idCardImageUrl
    selfieWithIdCardUrl.value = idCardInfo.selfieWithIdCardUrl
  }
})
...
<div class="form-control w-full flex flex-col items-center space-y-8 bg-gradient-to-br from-purple-100 via-pink-100 to-yellow-100 border border-gray-300 shadow-2xl rounded-3xl p-12 transition-transform duration-500 ease-in-out hover:shadow-2xl hover:scale-105">
  <h2 class="text-4xl font-bold text-gray-800 text-center tracking-wide mb-8">ID Card Information</h2>

  <!-- แสดงหมายเลขบัตรประชาชน -->
  <div class="flex flex-col items-center space-y-2">
    <span class="text-xl font-semibold text-gray-800">หมายเลขบัตรประชาชน</span>
    <div class="bg-white text-gray-900 text-2xl font-bold py-3 px-8 rounded-full shadow-lg tracking-wide">
      {{ citizenId || 'ไม่พบหมายเลขบัตรประชาชน' }}
    </div>
  </div>

  <!-- รูปบัตรประชาชน -->
  <div class="flex flex-col items-center space-y-4 mt-6">
    <span class="text-xl font-semibold text-gray-800">รูปบัตรประชาชน</span>
    <div v-if="idCardImageUrl" class="flex justify-center mt-4">
      <img :src="idCardImageUrl" alt="ID Card" class="w-80 h-80 object-cover rounded-xl shadow-xl border-4 border-purple-300 transition-transform duration-300 hover:scale-105" />
    </div>
  </div>

  <!-- รูปตัวเองถือบัตรประชาชน -->
  <div class="flex flex-col items-center space-y-4 mt-6">
    <span class="text-xl font-semibold text-gray-800">รูปตัวเองถือบัตรประชาชน</span>
    <div v-if="selfieWithIdCardUrl" class="flex justify-center mt-4">
      <img :src="selfieWithIdCardUrl" alt="Selfie with ID Card" class="w-80 h-80 object-cover rounded-xl shadow-xl border-4 border-purple-300 transition-transform duration-300 hover:scale-105" />
    </div>
  </div>
</div>
